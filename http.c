...
int http_copy_segment(int dst, int src, client_request_t *request) {
    ...
    // Считывание с указанием смещения
    ssize_t n = pread(src, buf, PAGE_SIZE, request->bytes_read);
    if (n < 0) {
        return ERR_READ; // Ошибка чтения
    }
    if (n == 0) {
        return ERR_EOF; // Конец файла
    }
    ssize_t written = write(dst, buf, n);
    if (written < 0) {
        return ERR_WRITE; // Ошибка записи
    }
    request->bytes_read += n;
    request->bytes_to_send -= written;
    if (n < PAGE_SIZE){
...
int http_serve_file(client_request_t *request, char *filename) {
    http_request_t *req = request->req;
    ...
    int rc = OK;
    // printf("state req 1 %d\n", request->state);
    if (request->state != STATE_SEND){
        http_write_status(req, RESPONSE_CODE_OK);

        rc = http_write_file_headers(req, filename, info);
        if (rc != 0) {
            request->state = STATE_ERROR;
            return rc;
        }
    ...
    // printf("state req 2 %d\n", request->state);
    rc = http_copy_segment(req->fd, fd, request);
    if (rc != 0) {
        close(fd);
        request->state = STATE_ERROR;
        return rc;
...
int http_handle( client_request_t *request) {
    if (strcmp(request->req->uri, "/") == 0) {
        return http_serve_file(request, "index.html");
    }
    char *filename = &(request->req->uri[1]);

    return http_serve_file(request, filename);
}
